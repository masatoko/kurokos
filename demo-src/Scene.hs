    ...
    alloc = do
      ...
      gui <- UI.newGui (UI.GuiEnv colset sdlAssets) $ do
        ...
        let size0 = V2 (Rpn "$width") (C 40)
            pos = V2 (C 0) (C 30)
        label <- UI.genSingle (Just "title") pos size0 =<< UI.newLabel "sysfont" "Kurokos DEMO"
        -- Buttons
        let size = V2 (Rpn "0.4 $width *") (C 40)
            pos1 = V2 (Rpn "0.3 $width *") (Rpn "0.2 $height *")
            pos2 = V2 (Rpn "0.3 $width *") (Rpn "0.2 $height * 50 +")
        button1 <- UI.genSingle (Just nameMain) pos1 size =<< UI.newButton "sysfont" "Next: Main Scene"
        button2 <- UI.genSingle (Just nameMouse) pos2 size =<< UI.newButton "sysfont" "Push: Mouse Scene"
        -- Image
        ...
        lbl' <- UI.genSingle (Just "label") (V2 (C 0) (C 0)) size' =<< UI.newLabel "sysfont" "---"
        btn' <- UI.genSingle (Just "button") (V2 (C 0) (Rpn "$height 2 /")) size' =<< UI.newButton "sysfont" "Button in Container"
        ...
        --
        btns <- mconcat <$> mapM (UI.genSingle Nothing (V2 (C 0) (C 0)) (V2 (Rpn "$width") (C 30)) <=< UI.newButton "sysfont" . T.pack . show) [1..(5::Int)]
